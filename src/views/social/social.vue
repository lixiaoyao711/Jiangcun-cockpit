<template>
  <div class="general"  key="social">
    <div class="main-title">
      <div class="main-title-text">聚焦民生 大爱蒋村</div>
    </div>
    <div class="map" id="socialMap"></div>
    <main>
      <Left />
      <Center />
      <Right />
...
import Left from '@/views/social/left';
import Right from '@/views/social/right';
import Center from '@/views/social/center';
import { indicatorSetTreMapList, IndicatorFeaturedWorkList, mapDigitalSocietyList } from '@/api/index';
...
const infoWindowData = computed(() => {
  console.log('infoWindow变了');
  return store.state.infoWindow;
});
let savePolygons = []
let savePonits = []
let polygons = []
...
let infoWindow = ref([]);
let tableData = ref([]);
watch(infoWindowData, (newValue, oldValue) => {
  // 确保勾选了AED点位才在地图上显示点位名称
  if (checkList.value.some((item) => item === 'AED点位')) {
    // infowidnow 的 innerHTML
    var infoWindowContent = `<div><label style="color:#fff">${newValue.addressName}</label></div>`;
    // 创建一个自定义内容的 infowindow 实例
    let infoWindow = new AMap.InfoWindow({
      position: newValue.point,
...
let checkbox = ref({
  ...
  婴幼儿成长驿站: {
    data: [],
    type: '',
    icon: 'image/yyeczyz.png',
  },
  幼儿园中小学: {
    data: [],
    type: '',
    icon: 'image/yeyzxx.png',
  },
  民生综合体: {
    data: [],
    type: '',
...
async function getList() {
  ...
  if (data) {
    isShowFooter.value = data['数字社会-特色工作-是否展示'].value === 'false' ? false : true;
    store.commit('setSocialData', data);
    // isShow.value = true;
  }
}
let map
