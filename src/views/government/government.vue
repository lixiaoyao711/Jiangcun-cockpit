<template>
  <div class="government">
    <div v-if="isShowMap === 1" class="map" id="govMap"></div>
    <main>
      <Left />
      <Center @tabChange="tabChange" />
      <Right />
      <featureWorkList v-if="isShowFooter" :tableData="tableData" :backImgChange="true" />
    </main>
  </div>
</template>
<script setup>
...
async function getList() {
  ...
  if (res) {
    ...
    // showData.value = res.rows[0];
    // if (showDataMap('数字政府', '1597485758657384449').value !== 'false') {
    //   titleChange(tableData.value[3], 3);
    // }
  }
  const { data } = await indicatorSetTreMapList(obj);
  if (data) {
    isShowFooter.value = data['数字政府-特色工作-是否展示'].value === 'false' ? false : true;
...
function tabChange(val) {
  ...
  if (val === 1) {
    ...
    // if (showDataMap('数字政府', '1597485758657384449').value !== 'false') {
    //   titleChange(tableData.value[0], 0);
    // }
  }
}
let tableData = ref([]);
...
async function getMapData() {
  ...
  if (res) {
    mapPath = res.rows;
    formatMapData(mapPath, checkbox.value);
    console.log('formatMapData', mapPath, checkbox.value);
  }
}
